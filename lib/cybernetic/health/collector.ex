defmodule Cybernetic.Health.Collector do
  ...
  @retention_period 3_600_000  # 1 hour in milliseconds
  ...
  def handle_call({:history, duration_ms}, _from, state) do
    ...
    recent_history = Enum.filter(state.history, fn {timestamp, _} ->
  ...
  def handle_call({:aggregate, metric_name, duration_ms}, _from, state) do
    ...
    aggregate = if Enum.empty?(values) do
      %{count: 0}
    else
      %{
        count: length(values),
  ...
  def handle_info(:collect, state) do
    ...
    pruned_history = Enum.filter(new_history, fn {ts, _} ->
      ts > cutoff_time
    end)
