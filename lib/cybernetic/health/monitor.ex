defmodule Cybernetic.Health.Monitor do
  ...
  defp check_vsm_layers do
    ...
    results = Enum.map(layers, fn layer ->
      process_name = case layer do
  ...
  defp check_disk_space do
    case :disksup.get_disk_data() do
      [_ | _] = disks ->
        ...
        critical = Enum.any?(disks, fn {_mount, _size, usage} ->
          usage > 90
        end)
  ...
  defp determine_overall_status(checks, failures) do
    ...
    critical_unhealthy = Enum.any?(critical_components, fn comp ->
      case checks[comp] do
        :unhealthy -> Map.get(failures, comp, 0) >= @unhealthy_threshold
