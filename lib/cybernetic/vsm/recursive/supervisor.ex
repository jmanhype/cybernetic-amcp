defmodule Cybernetic.VSM.Recursive.Supervisor do
      ...
      [{^child_id, pid, old_quotas, spec}] ->
        ...
        case validate_quotas(merged_quotas) do
          :ok ->
            ...
            Logger.info("Scaled child VSM #{child_id}: #{inspect(old_quotas)} -> #{inspect(merged_quotas)}")
            :ok
  ...
  defp build_child_spec(child_id, spec, quotas) do
    ...
    %{
      id: child_id,
      start: {Cybernetic.VSM.Recursive.ChildVSM, :start_link, [
        [
          name: child_id,
          quotas: quotas,
          parent: self(),
...
defmodule Cybernetic.VSM.Recursive.ChildVSM do
  ...
  def init(opts) do
    ...
    children = [
      ...
      # Mini S3 - Control
      {Cybernetic.VSM.Recursive.MiniS3,
        name: :"#{child_id}_s3",
        namespace: amqp_namespace
      },
      
      # Mini S4 - Intelligence
      {Cybernetic.VSM.Recursive.MiniS4,
        name: :"#{child_id}_s4",
        namespace: amqp_namespace
      },
...
defmodule Cybernetic.VSM.Recursive.ResourceGovernor do
  ...
  def handle_info(:check_resources, state) do
    ...
    if memory_mb > state.quotas.memory_mb do
      Logger.warning("Child VSM #{state.child_id} exceeding memory quota: #{memory_mb}MB > #{state.quotas.memory_mb}MB")
      state = Map.update!(state, :violations, &(&1 + 1))
      
      if state.violations > 3 do
        Logger.error("Child VSM #{state.child_id} repeatedly violating quotas, requesting termination")
