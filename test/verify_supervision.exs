defmodule VerifySupervision do
  ...
  def test_supervisor_strategy do
    ...
    if s1_before do
      ...
      Process.sleep(500)  # Give supervisor time to restart
      ...
      IO.puts "\nAfter S1 crash:"
      IO.puts "  S1: #{inspect(s1_before)} → #{inspect(s1_after)} #{if s1_after && s1_after != s1_before, do: "✅ RESTARTED", else: "❌"}"
      IO.puts "  S2: #{inspect(s2_before)} → #{inspect(s2_after)} #{if s2_after == s2_before, do: "✅ UNCHANGED", else: "❌ CHANGED"}"
      IO.puts "  S3: #{inspect(s3_before)} → #{inspect(s3_after)} #{if s3_after == s3_before, do: "✅ UNCHANGED", else: "❌ CHANGED"}"
      IO.puts "  S4: #{inspect(s4_before)} → #{inspect(s4_after)} #{if s4_after == s4_before, do: "✅ UNCHANGED", else: "❌ CHANGED"}"
      IO.puts "  S5: #{inspect(s5_before)} → #{inspect(s5_after)} #{if s5_after == s5_before, do: "✅ UNCHANGED", else: "❌ CHANGED"}"
      ...
      if s5_before2 do
        ...
        IO.puts "  S1: #{inspect(s1_after)} → #{inspect(s1_after2)} #{if s1_after2 != s1_after, do: "⚠️  RESTARTED", else: "✅ UNCHANGED"}"
        IO.puts "  S2: #{inspect(s2_after)} → #{inspect(s2_after2)} #{if s2_after2 != s2_after, do: "⚠️  RESTARTED", else: "✅ UNCHANGED"}"
        IO.puts "  S3: #{inspect(s3_after)} → #{inspect(s3_after2)} #{if s3_after2 != s3_after, do: "⚠️  RESTARTED", else: "✅ UNCHANGED"}"
        IO.puts "  S4: #{inspect(s4_after)} → #{inspect(s4_after2)} #{if s4_after2 != s4_after, do: "⚠️  RESTARTED", else: "✅ UNCHANGED"}"
        IO.puts "  S5: #{inspect(s5_before2)} → #{inspect(s5_after2)} #{if s5_after2 && s5_after2 != s5_before2, do: "✅ RESTARTED", else: "❌"}"
      end
    end
