defmodule Cybernetic.Intelligence.CEP.WorkflowHooksTest do
  ...
  describe "register/2" do
    test "registers a valid hook and returns hook_id" do
      ...
      assert String.length(hook_id) == 32  # 16 bytes hex = 32 chars
    end
    ...
    test "respects max_hooks limit" do
      ...
      {:ok, _} = WorkflowHooks.register(%{pattern: %{}, action: {:log, :info}}, server: :limited_hooks)
      {:ok, _} = WorkflowHooks.register(%{pattern: %{}, action: {:log, :info}}, server: :limited_hooks)
  ...
  describe "unregister/2" do
    test "removes a registered hook" do
      {:ok, hook_id} = WorkflowHooks.register(%{pattern: %{}, action: {:log, :info}}, server: :test_hooks)
  ...
  describe "process_event_sync/2" do
    ...
    test "matches with :in operator" do
      {:ok, _} = WorkflowHooks.register(
        %{name: "in_matcher", pattern: %{status: {:in, [:active, :pending]}}, action: {:log, :info}},
        server: :test_hooks
      )
      ...
      {:ok, triggered} = WorkflowHooks.process_event_sync(%{status: :inactive}, server: :test_hooks)
      assert triggered == 0
    end

    test "matches with :contains operator" do
      {:ok, _} = WorkflowHooks.register(
        %{name: "contains_matcher", pattern: %{message: {:contains, "error"}}, action: {:log, :info}},
        server: :test_hooks
      )

      {:ok, triggered} = WorkflowHooks.process_event_sync(%{message: "An error occurred"}, server: :test_hooks)
      assert triggered == 1
    end
    ...
    test "matches nested fields with dot notation" do
      ...
      {:ok, triggered} = WorkflowHooks.process_event_sync(
    ...
    test "matches severity comparison" do
      {:ok, _} = WorkflowHooks.register(
        %{name: "severity_matcher", pattern: %{severity: {:gte, "high"}}, action: {:log, :info}},
        server: :test_hooks
      )

      {:ok, triggered} = WorkflowHooks.process_event_sync(%{severity: "critical"}, server: :test_hooks)
      assert triggered == 1
  ...
  describe "list_hooks/1" do
    test "returns all registered hooks" do
      {:ok, _} = WorkflowHooks.register(%{name: "hook1", pattern: %{}, action: {:log, :info}}, server: :test_hooks)
      {:ok, _} = WorkflowHooks.register(%{name: "hook2", pattern: %{}, action: {:log, :info}}, server: :test_hooks)
  ...
  describe "stats/1" do
    test "returns statistics" do
      {:ok, _} = WorkflowHooks.register(%{pattern: %{}, action: {:log, :info}}, server: :test_hooks)
