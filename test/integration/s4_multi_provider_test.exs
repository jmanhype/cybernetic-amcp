defmodule Cybernetic.Integration.S4MultiProviderTest do
  ...
  @moduletag timeout: 120_000  # 2 minutes for API calls
  ...
  describe "Multi-Provider Episode Routing" do
    test "routes policy_review episodes to Anthropic primary chain" do
      episode = create_test_episode(:policy_review, %{
    ...
    test "routes code_gen episodes to OpenAI primary chain" do
      episode = create_test_episode(:code_gen, %{
    ...
    test "routes anomaly_detection to balanced chain" do
      episode = create_test_episode(:anomaly_detection, %{
  ...
  describe "Budget Management Integration" do
    test "budget tracking for S4 LLM requests" do
      ...
      Logger.info("✓ Initial budget status: #{initial_status.remaining}/#{initial_status.limit} tokens available")
      ...
      Logger.info("✓ Final budget utilization: #{Float.round(final_status.utilization * 100, 2)}%")
    end
  end
  ...
  describe "Complete S4 Analysis Flow" do
    ...
    test "end-to-end episode analysis with provider fallback" do
      ...
      case Service.analyze(episode) do
        {:ok, analysis} ->
          ...
          Logger.info("  - Tokens used: #{analysis.tokens.input} input, #{analysis.tokens.output} output")
    ...
    test "local analysis with Ollama provider" do
      episode = create_test_episode(:privacy_review, %{
      ...
      case Ollama.analyze_episode(episode, model: "deepseek-r1:7b") do
        {:ok, analysis} ->
          ...
          assert analysis.usage.cost_usd == 0.0  # Local processing = no cost
        ...
        {:error, :server_unavailable} ->
          Logger.warning("⚠ Ollama server not available (install with: ollama pull deepseek-r1:7b)")
  ...
  describe "Circuit Breaker and Resilience" do
    test "circuit breaker behavior on provider failures" do
      ...
      results = Enum.map(chain, fn provider ->
  ...
  describe "Telemetry and Observability" do
    test "telemetry events are emitted during analysis" do
      ...
      episode = create_test_episode(:root_cause, %{
  ...
  defp apply_provider_with_circuit_breaker(provider, episode) do
    ...
    case provider do
      :anthropic -> 
        if System.get_env("ANTHROPIC_API_KEY"), do: {:ok, %{provider: :anthropic}}, else: {:error, :no_api_key}
      :openai -> 
        if System.get_env("OPENAI_API_KEY"), do: {:ok, %{provider: :openai}}, else: {:error, :no_api_key}
      :ollama -> 
        case Ollama.health_check() do
