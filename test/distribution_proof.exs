defmodule DistributionProof do
  ...
  def test_single_node_failure do
    ...
    if s1_pid do
      ...
      IO.puts "  S1: #{inspect s1_pid} -> #{inspect new_s1_pid} #{if new_s1_pid != s1_pid, do: "✅ RESTARTED", else: "❌ DEAD"}"
      IO.puts "  S2: #{inspect s2_pid} -> #{inspect new_s2_pid} #{if new_s2_pid == s2_pid, do: "✅ UNCHANGED", else: "⚠️ RESTARTED"}"
      IO.puts "  S3: #{inspect s3_pid} -> #{inspect new_s3_pid} #{if new_s3_pid == s3_pid, do: "✅ UNCHANGED", else: "⚠️ RESTARTED"}"
      IO.puts "  S4: #{inspect s4_pid} -> #{inspect new_s4_pid} #{if new_s4_pid == s4_pid, do: "✅ UNCHANGED", else: "⚠️ RESTARTED"}"
      IO.puts "  S5: #{inspect s5_pid} -> #{inspect new_s5_pid} #{if new_s5_pid == s5_pid, do: "✅ UNCHANGED", else: "⚠️ RESTARTED"}"
