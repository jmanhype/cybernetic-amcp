...
try do
  ...
  case Cybernetic.Core.MCP.Hermes.Registry.invoke_tool("vsm_query", %{system: "s1", query: "status"}) do
...
try do
  ...
  case Cybernetic.Core.CRDT.GraphQueries.run_query(query) do
    {:ok, _} -> IO.puts("  ✓ Graph query executed")
    {:error, :not_found} -> IO.puts("  ✓ Graph query returned not_found (expected for empty graph)")
    {:error, reason} -> IO.puts("  ❌ Graph query failed: #{inspect(reason)}")
  end
rescue
...
try do
  ...
  result = Cybernetic.VSM.System1.MessageHandler.handle_message(
    message.operation,
    message.payload,
    %{source_node: node()}
  )
